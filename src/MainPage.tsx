...
const Button1 = ({ label, onClick }: ButtonProps) => (
...
const Main = () => {
  ...
  const handleSaveClick = () => {
    setEditing(false);
    // do something with the saved text, like submitting a form
  };
  ...
  // };
  type ButtonType = 'edit' | 'save' | 'cancel';
  // const [buttonType, setButtonType] = useState<ButtonType>('edit');
  ...
  const items: MenuProps['items'] = [
    ...
    },
    {
      key: '2',
      label: (
        <div onClick={() => setIsToggled(!isToggled)}>Tạo văn bản</div>
      ),
    },
    {
      key: '3',
  ...
  const onClick: MenuProps['onClick'] = async (e) => {
   ...
   //   console.log(request)
      ...
      k = {
        ...
        <>
            { editing && ( <>
                <Button1 label="Save"  onClick={() =>{ save(record.key); handleSaveClick(); }}/>
                <Button1 label="Cancel" onClick={handleCancelClick}/>
              </>
            )}
            <Button1 label="Edit" onClick={() => { edit(record); handleEditClick();}}/>
            </>
      //   <>
      //   { (isEditing(record) && buttonType === 'save')  ? (
      //     <span>
      ...
      //     </span>
      //   ) : (
      //     <Typography.Link disabled={editingKey !== ''} onClick={() => {edit(record); setButtonType('save');}}>
      //       Edit
      //     </Typography.Link>
      //   )}
      // </>
